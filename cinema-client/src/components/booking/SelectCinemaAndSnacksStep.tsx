...
export type SelectedSnackInfo = { // This type might be better in a shared types file
  name: string;
  price: number;
  quantity: number;
};
...
const SelectCinemaAndSnacksStep: React.FC<SelectCinemaAndSnacksStepProps> = ({
  ...
  const { cinemas, loading: cinemasLoading, error: cinemasError } = useMovieCinemas(movieId, city);
  ...
  const handleSnackQuantityChange = (snack: TSnack, quantity: number) => {
    ...
    if (newQuantity === 0) {
      ...
      updated[snack._id!] = { name: snack.name, price: snack.price, quantity: newQuantity };
    }
    onSnacksUpdate(updated);
  };

  const totalSnackCost = useMemo(() => {
    return Object.values(selectedSnacks).reduce((total, snack) => total + snack.price * snack.quantity, 0);
  }, [selectedSnacks]);

  if (!movieId || !city) { /* ... cinema loading/error/empty states ... */ }
  if (cinemasLoading) return <Spinner size="medium" />;
  if (cinemasError) return <div className={styles.errorMessage}>Error loading cinemas: {cinemasError}</div>;
      ...
      <div className={styles.cinemaSelectSection}>
        <select
          ...
          {cinemas.map((cinema: TCinema) => (
            <option className={styles.optionStyle} key={cinema.id} value={cinema.id}>
              {cinema.name}
            </option>
          ))}
        </select>
        {cinemas && cinemas.length === 0 && !cinemasLoading && (
          <div className={styles.infoMessage}>No cinemas found for this selection.</div>
        )}
      </div>
